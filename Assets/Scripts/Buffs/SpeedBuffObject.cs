...
public class SpeedBuffObject : MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    private float _speedBuffAmount;
    [SerializeField]
    private float _time;
    [SerializeField]
    ...
    void Start()
    {
        ...
        if (_speedBuff == null)
        {
            _speedBuff = FindObjectOfType<SpeedBuff>();
        }
        if (_icon == null)
        {
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.layer == _layerPlayer)
        {
            if (_speedBuff == null)
            {
                _speedBuff = collision.GetComponent<SpeedBuff>();
            }
            _speedBuff.StartSpeedUp(_speedBuffAmount, _time);
            _audioSource.Play();
